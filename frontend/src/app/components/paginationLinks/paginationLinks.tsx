...
const PaginationLinks: FC<Props> = ({contentCount}) => {
    ...
    return (
        <div className={classes.paginationLinks}>
            ...
            {currentPage === 4 &&
                <Link href={`?page=${2}`}
                      className={classes.paginationLinks__link}>2</Link>
            }
            {currentPage > 4 &&
                <div className={classes.paginationLinks__link}>...</div>
            }
            {currentPage !== 1 && currentPage !== 2 &&
                <Link href={`?page=${currentPage - 1}`}
                      className={classes.paginationLinks__link}>{currentPage - 1}</Link>
            }
            {currentPage !== 1 && currentPage !== pagesCount &&
                <div className={classes.paginationLinks__link}>{currentPage}</div>
            }
            ...
            {currentPage === pagesCount - 3 &&
                <Link href={`?page=${pagesCount - 3}`}
                      className={classes.paginationLinks__link}>{pagesCount - 3}</Link>
            }
            <Link href={`?page=${pagesCount}`} className={classes.paginationLinks__link}>{pagesCount}</Link>
        </div>
    );
};
